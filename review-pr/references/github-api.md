...
6. **Verify line numbers against the source file, not the diff**: Diff hunk headers (e.g., `@@ -10,6 +12,20 @@`) show approximate ranges and can be misleading — they don't account for surrounding context lines, doc comments, or blank lines. Before setting `line` or `start_line` in a comment, run `cat -n /tmp/pr-review/source/{file}` or `grep -n '{target code}' /tmp/pr-review/source/{file}` to confirm the exact line number. Getting this wrong causes comments to appear on the wrong line (e.g., on a doc comment instead of the method signature).
